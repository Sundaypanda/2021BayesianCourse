...
利用包括气候数据和土壤中铀含量测量在内的县级预测指标W，表征j县地质特征的指标$ \delta_k(j) $为K = 19类型之一。最后，根据数据估算了19种地质类型的$ \delta_k $系数，$$ \delta_k \widetilde{} N(0,\kappa^2), &emsp; for &emsp; geologic &emsp; type &emsp; k = 1,...,K, $$
...
决策树由三个分支组成。在每个分支，我们以美元为单位评估预期损失，使用$ D_r = 2000/(R_{action}−R_{remed}) $作为额外家庭氡暴露的每pCi/L等效成本，将氡暴露(超过30年)换算成美元价值。在下面的表达式中，我们设$ R = e^\theta $为所考虑的家中未知的氡暴露水平；先验分布和后验分布为$ \theta = \log R $的正态分布。
...
如果我们愿意对所有县进行简化假设，即$ \sigma = \log(1.2) $和$ S = \log(2.3) $，那么我们可以通过给出阈值水平$ M_{low} $和$ M_{high} $来总结决策建议，如果$ M > M_{high} $，则首选决策1（立即补救），如果$ M < M_{low} $，则首选决策2（不监测或补救），如果$ M \in [M_{low},M_{high}] $。图9.5显示了这些截止值作为$ R_{action} $的函数，因此显示了推荐决策作为($ R_{action} $，e^M)的函数。例如，将$ R_{action}=4pCi/L $设置为以下基于$ e^M $的建议，即基于您所在的县和房屋类型的家庭氡的先前GM：
